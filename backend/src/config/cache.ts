...
// Check if Redis is configured (optional for development)
...
if (redis) {
  ...
  logger.info('Redis is disabled - running without cache (development mode)');
}
...
export class CacheManager {
  ...
  async get<T = any>(key: string): Promise<T | null> {
    if (!redis) {
      logger.debug(`Cache disabled - returning null for: ${key}`);
      return null;
    }
    try {
  ...
  async del(key: string): Promise<void> {
    if (!redis) {
      logger.debug(`Cache disabled - skipping delete: ${key}`);
      return;
    }
    try {
      await redis.del(key);
  ...
  async increment(key: string, ttl?: number): Promise<number> {
    if (!redis) {
      logger.debug(`Cache disabled - returning 0 for increment: ${key}`);
      return 0;
    }
    try {
  ...
  async setNX(key: string, value: any, ttl?: number): Promise<boolean> {
    if (!redis) {
      logger.debug(`Cache disabled - returning false for setNX: ${key}`);
      return false;
    }
    try {
      const serializedValue = JSON.stringify(value);
...
export const closeRedis = async (): Promise<void> => {
  if (!redis) {
    logger.info('Redis not enabled - no connection to close');
    return;
  }
  try {
    await redis.quit();
